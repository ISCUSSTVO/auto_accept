...
if __name__ == "__main__":
    ...
    def update_message(message):
        ...
        message_box.see(tk.END)  # Прокручиваем вниз к последнему сообщению
    ...
    def main_loop():
        ...
        if loc[0].size > 0:
            if not button_found:  # Если кнопка найдена и флаг не установлен
                update_message("Нажал")
                pyautogui.press('enter')  # Нажимаем Enter
                button_found = True  # Устанавливаем флаг, что кнопка найдена
        else:
            if button_found:
                update_message("Поиск") 
                button_found = False  # 
        root.after(500, main_loop)  # Запускаем функцию снова через 1000 мс (1 секунда)
